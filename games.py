...
# ----------------------------GETTING GAMES URLs-------------------------------
    ...
    def parse(self, response):
        ...
        num_of_games_on_page = len(response.css('a.title::attr(href)').getall())
        end = num_of_games_on_page if num_of_games_on_page <= self.items_per_page else self.items_per_page

        for x in range(0, end):
            yield {
             #Extracts the link of the game and stores-rw-r--r--  1 selimtawfik  staff  2624 20 Apr 15:24 gm.jl
             'f': response.css('a.title::attr(href)')[x].get()
            }
...
# ---------------------------FOLLOW TO THE NEXT PAGE-------------------------
        ...
        NEXT_PAGE_SELECTOR = 'span.flipper.next a.action::attr(href)'
